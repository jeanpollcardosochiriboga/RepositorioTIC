...
cmd_start() {
    ...
    # --- guard anti doble-pulsado / fuera de secuencia --------------------------
    # La consola la opera alguien que solo aprieta botones: hay que aguantar dos
    # toques seguidos o un orden equivocado sin lanzar colectores duplicados ni
    # pisar el estado. flock serializa arranques concurrentes; si ya hay una
    # sesion con colectores vivos, se avisa amablemente y se sale sin tocar nada.
    exec 9>"$HERE/.session_start.lock"
    if ! flock -n 9; then
        echo "Ya hay un arranque en curso. Espera unos segundos."; exit 0
    fi
    if [ -f "$STATE_FILE" ]; then
        local act_scn act_sdir rpid
        act_scn="$(. "$STATE_FILE"; echo "$SCENARIO")"
        act_sdir="$(. "$STATE_FILE"; echo "$SDIR")"
        rpid="$(cat "$act_sdir/router_collector.pid" 2>/dev/null || echo "")"
        if [ -n "$rpid" ] && kill -0 "$rpid" 2>/dev/null; then
            if [ "$act_scn" = "$scenario" ]; then
                echo "Ya estas midiendo $scenario. No hace falta iniciar de nuevo."
            else
                echo "Hay una medicion activa de $act_scn. Pulsa TERMINAR antes de medir $scenario."
            fi
            exit 0
        fi
        echo "[start] sesion previa sin colectores vivos; limpio el estado viejo"
        rm -f "$STATE_FILE"
    fi
    ...
    # Rotar los CSV que Node-RED escribe en modo append a metrics_out
    # (survey, backend_latency, dns_queries). NO estan atados a la sesion, asi
    # que sin esto se acumulan entre demos y arrastran datos de dias viejos
    # (ver METRICAS.md). Se deja solo la cabecera -> cada sesion captura su
    # propia ventana. Los crea el contenedor como root; sudo -n (NOPASSWD en el
    # Pi) trunca sin colgar la sesion no interactiva. Si falla, solo avisa.
    if [ -n "$pi_scn_out" ]; then
        for _f in backend_latency.csv survey.csv dns_queries.csv; do
            local _p="${pi_scn_out}/${_f}"
            [ -f "$_p" ] || continue
            if sudo -n sh -c "head -1 '$_p' > '$_p.tmp' && mv '$_p.tmp' '$_p'" 2>/dev/null; then
                echo "[start] rotado ${_f} (solo cabecera)"
            else
                echo "[start] aviso: no se pudo rotar ${_f} (revisar permisos/sudo)"
            fi
        done
    fi
...
cmd_end() {
    # Idempotente: si no hay sesion activa (TERMINAR pulsado de mas o antes de
    # iniciar), avisa amablemente en vez de soltar un error tecnico.
    if [ ! -f "$STATE_FILE" ]; then
        echo "No hay ninguna medicion activa. (Nada que cerrar.)"; return 0
    fi
    read_state
